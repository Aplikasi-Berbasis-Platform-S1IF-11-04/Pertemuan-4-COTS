...
## Deskripsi Aplikasi
Aplikasi Arvan Hiking merupakan sistem informasi sederhana berbasis web yang digunakan untuk mengelola data peralatan hiking dan pendakian. Aplikasi ini mengimplementasikan konsep CRUD (Create, Read, Update, Delete) menggunakan arsitektur client-server dengan pendekatan REST API.
...
Aplikasi Arvan Hiking merupakan sistem manajemen data peralatan hiking berbasis web yang dibangun menggunakan Node.js dan Express.

Fitur utama:

<<<<<<< HEAD
- Menampilkan data peralatan hiking

- Menambahkan data alat baru

- Mengedit data peralatan
...
<<<<<<< HEAD
        $("#tabelProduk tbody").on("click", ".btn-hapus", function () {
          ...
          if (
            confirm(
              "Tindakan ini tidak dapat dibatalkan. Hapus peralatan ini dari database?",
            )
          ) {
            $.ajax({
              url: `/api/products/${id}`,
